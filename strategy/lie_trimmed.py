...
from .aggregate import aggregate_trimmed_avg, weighted_loss_avg
...
class LIE_trimmed(Strategy):
    ...
    def aggregate_fit(
        ...
        grad_output: NDArrays = []

        m = len(self.malicious_list)
        n = self.min_available_clients
        ...
        for layer_updates in zip(*users_grads):
            lay = cp.asarray(layer_updates)
            grads_mean = cp.mean(lay, axis=0)
            grads_stdev = cp.var(lay, axis=0) ** 0.5
            derive = num_std * grads_stdev[:]
            print(derive)
            grads_mean[:] += derive
            grad_output.append(cp.asnumpy(grads_mean))
            del grads_mean
            del grads_stdev
            del lay
            del derive
        ...
        for client, fit_res in results:
            cid = client.cid
            if eval(cid) in self.malicious_list:
                weights_results.append((grad_output, fit_res.num_examples))
            else:
